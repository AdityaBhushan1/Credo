...
async def channel_input(ctx, check, timeout=120, delete_after=False):
    try:
        ...
        raise exceptions.InputError(f"{emote.error} | You failed to select a channel in time. Try again!")

    else:
        if not channel.permissions_for(ctx.me).read_messages:
            raise exceptions.InputError(f"{emote.error} | I don't have send messages permissions in {channel.mention}.")
...
async def role_input(ctx, check, timeout=120, hierarchy=True, delete_after=False):
    try:
        ...
        raise exceptions.InputError(f"{emote.error} | You failed to select a role in time. Try again!")
        ...
        if hierarchy is True:
            ...
            if role.managed:
                raise exceptions.InputError(f"{emote.error} | Role is an integrated role and cannot be added manually.")
...
async def member_input(ctx, check, timeout=120, delete_after=False):
    try:
        ...
        raise exceptions.InputError(f"{emote.error} | You failed to mention a member in time. Try again!")
...
async def integer_input(ctx, check, timeout=120, limits=(None, None), delete_after=False):
    ...
    try:
        ...
        raise exceptions.InputError(f"{emote.error} | You failed to select a number in time. Try again!")
    else:
        if delete_after:
            await safe_delete(message)
...
async def time_input(ctx, check, timeout=120, delete_after=False):
    try:
        ...
        raise exceptions.InputError(f"{emote.error} | Timeout, You have't responsed in time. Try again!")
    else:
        try:
            ...
            raise exceptions.InputError(f"{emote.error} | This isn't valid time format.")
...
async def string_input(ctx, check, timeout=120, delete_after=False):
    try:
        ...
        raise exceptions.InputError(f"{emote.error} | Took too long. Good Bye.")  # This would sound cooler.
    else:
        if delete_after:
            await safe_delete(message)
